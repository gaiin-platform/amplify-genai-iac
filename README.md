...
This repository contains very opinionated Terraform modules for setting up AWS infrastructure components for a scalable infrastructure to deploy AmplifyGenAI into. The infrastructure includes a load balancer, an Elastic Container Registry (ECR) repository, and an Elastic Container Service (ECS) cluster with Fargate, along with a basic ECR deployment pipeline.
